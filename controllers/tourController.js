...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  // /tours-within/10/center/35.749063,51.481044/unit/km

  const { distance, latlon, unit } = req.params;
  const [lat, lon] = latlon.split(',');

  // radius in radiant format, radius / earth radius
  const radius = unit === 'mi' ? distance / 3963.2 : distance / 6378.1;

  if (!lat || !lon) {
    next(
      new AppError(
        'Please provide latitude or longitude in the format lat,lon.',
        400
      )
    );
  }
