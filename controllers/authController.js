...
const createSendToken = (user, statusCode, res) => {
  ...
  // remove password from showing after creating a user
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // we have deselected the password in model so we have to explicitly select it
  const user = await User.findOne({ email }).select('+password');

  // we check like this to be vague and the attacker does not know which one is incorrect
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 2) Verification token - payload has not been manipulated
  const decoded = await promisify(jwt.verify)(token, process.env.JWT_SECRET);
  // decoded:
  /* { id: '5d3d7bf41e05bd24589f3e0a',
  iat: 1564310718,
  exp: 1572086718 } */
...
// this middleware takes arguments
